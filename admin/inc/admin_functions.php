...
// Receives new admin data from form, create new admin user, returns all admin users with their roles.
function createAdmin($request_values){
	...
	$request_values['role'] ? $role = esc($request_values['role']) : null;
...
//  Fetches the admin from database, sets admin fields on form for editing.
...
function updateAdmin($request_values){
	...
	isset($request_values['role']) ? $role = $request_values['role'] : null;
...
function createTopic($request_values){
	...
	if (count($errors) == 0) {
		$query = "INSERT INTO topics (topic, slug) 
...
function updateTopic($request_values) {
	...
	// register topic if there are no errors in the form
	if (count($errors) == 0) {
		$query = "UPDATE topics SET topic='$topic_name', slug='$topic_slug' WHERE topic_id=$topic_id";
		mysqli_query($conn, $query);
